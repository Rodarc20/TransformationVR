...
ABotonRotar::ABotonRotar() {
	PrimaryActorTick.bCanEverTick = true;
    TareaAsociada = ETransformacionTarea::ERotar;
    ...
    if (BotonAsset.Succeeded()) {
        ...
        //static ConstructorHelpers::FObjectFinder<UMaterial> BotonMaterialAsset(TEXT("Material'/Game/Trasnformation/Assets/Meshes/Botones/BotonMaterial_6.BotonMaterial_6'"));//de usar este creo que debo crear un obtener un  material y ponerselo, este tiene el pivot en el centro de la esfera
        static ConstructorHelpers::FObjectFinder<UMaterial> BotonMaterialAsset(TEXT("Material'/Game/Trasnformation/Assets/Meshes/Botones/MaterialBotonC_3N.MaterialBotonC_3N'"));//de usar este creo que debo crear un obtener un  material y ponerselo, este tiene el pivot en el centro de la esfera
    ...
    static ConstructorHelpers::FObjectFinder<UMaterial> BotonMaterialPAsset(TEXT("Material'/Game/Trasnformation/Assets/Meshes/Botones/MaterialBotonC_3.MaterialBotonC_3'"));//de usar este creo que debo crear un obtener un  material y ponerselo, este tiene el pivot en el centro de la esfera
    if (BotonMaterialPAsset.Succeeded()) {
        MaterialPresionado = BotonMaterialPAsset.Object;
    }

    Boton->OnComponentBeginOverlap.AddDynamic(this, &ABotonRotar::OnBeginOverlapBoton);
    Boton->OnComponentEndOverlap.AddDynamic(this, &ABotonRotar::OnEndOverlapBoton);
...
void ABotonRotar::BeginPlay() {
    Super::BeginPlay();
}

void ABotonRotar::Tick(float DeltaTime) {
...
void ABotonRotar::OnBeginOverlapBoton(UPrimitiveComponent * OverlappedComponent, AActor * OtherActor, UPrimitiveComponent * OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult & SweepResult) {
...
void ABotonRotar::OnEndOverlapBoton(UPrimitiveComponent * OverlappedComponent, AActor * OtherActor, UPrimitiveComponent * OtherComp, int32 OtherBodyIndex) {
    if ((OtherActor != nullptr) && (OtherActor != this) && (OtherComp != nullptr) && (OtherActor != GetOwner())) { //no es necesario el ultimo, solo para este caso particular en el que no quiero que el propio conejo active esta funconalidad
        AVRPawn * const VRPawn = Cast<AVRPawn>(OtherActor);
        if (VRPawn && !VRPawn->IsPendingKill()) {
            UCapsuleComponent * const ColisionController = Cast<UCapsuleComponent>(OtherComp);//para la casa no necesito verificar que haya tocado su staticmesh
